...
const App: React.FC = () => {
  ...
  }, [category, lang, hasKey, isKeyChecking, showAnnouncements]);
  ...
  return (
    <div className={cn(
          ...
          {Object.values(Category).map(cat => (
            <button 
              ...
              onClick={() => {
                ...
                setShowAnnouncements(cat === Category.ANNONCES);
                if (category !== cat) {
                    setCategory(cat);
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-10 w-full">
        {showIslamCourse ? (
        ...
        ) : showAnnouncements || category === Category.ANNONCES ? (
          <AnnouncementsView />
        ) : loading && articles.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 animate-in fade-in duration-500">
            ...
            {category === Category.UNES && <SponsorBanner />}
          </div>
        )}
      </main>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex gap-2">
                ...
                <button onClick={() => handleDownloadAudio(selected)} className={`p-5 md:p-6 border-2 font-black uppercase tracking-widest flex items-center justify-center rounded-full transition-all ${isReadingMode ? 'border-white text-white hover:bg-white hover:text-black' : 'border-black text-black hover:bg-zinc-50'}`}>
                    <svg className="w-5 h-5 md:w-6 md:h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm3-7.707a1 1 0 011.414 0L9 10.586V3a1 1 0 112 0v7.586l1.293-1.293a1 1 0 111.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                </button>
              </div>
